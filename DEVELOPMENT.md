...
- **Piano (鋼琴專區)**：整合多重來源。核心演進為**「資料館藏化 (Curated Collection)」** (v19.5.7) —— 透過 `is_piano_solo` 旗標精確鎖定 [piano.php](https://www.ufret.jp/piano.php) 的精選區域；若非從專屬區域抓取的鋼琴標籤曲目將不再自動進入 Piano 分頁，確保該區域僅保留最優質的 8 首經典（或其他官方精選），並在啟動時執行「資料庫大掃除」自動剔除舊有的 Unknown 雜訊。
